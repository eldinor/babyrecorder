...
WebGPUEngine.prototype._debugPopGroup = function () {
    ...
    if (this._currentRenderPass) {
        if (this._debugStackRenderPass.length === 0) {
            if (this._showGPUDebugMarkersLog) {
                Logger.Log(`[${this.frameId}] Popping debug group on current render pass '${this._currentRenderPass.label}' but no group was pushed on it, so recording a pending pop to be executed after ending the current render pass`);
            }
